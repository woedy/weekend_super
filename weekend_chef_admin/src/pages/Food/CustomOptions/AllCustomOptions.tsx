...
import AddCustomOptionModal from './modals/AddCustomOptionModal';
...
const AllCustomOptions = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${baseUrl}api/orders/get-all-custom-options/?search=${encodeURIComponent(
          search,
        )}&page=${page}&option_type=${optionType}`,
        {
          headers: {
  ...
  const handleDelete = async (itemId) => {
    ...
    try {
      const response = await fetch(`${baseUrl}api/orders/delete-custom-option/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Token ${userToken}`,
  ...
  const handleArchive = async (itemId) => {
    ...
    try {
      const response = await fetch(`${baseUrl}api/orders/archive-custom-option/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Token ${userToken}`,
  ...
  return (
    <div>
      <Breadcrumb pageName="Custom Options" />
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="col-span-2 rounded-sm border border-stroke  shadow-default dark:border-strokedark dark:bg-boxdark">
          ...
          <div className="grid grid-cols-3 gap-5 py-6 px-4 md:px-6 xl:px-7.5">
            <input
              type="text"
            ...
            <Link to={'/archived-custom-options/'}>
              <button className="flex w-full justify-center rounded p-3 font-medium text-black hover:bg-opacity-90">
                Archived
              </button>
            </Link>
          </div>
          ...
          {customOptions
            ? customOptions.map((customOption) => (
                ...
                >
                  ...
                  <div className="col-span-1 hidden items-center sm:flex mr-4">
                    <p className="text-sm text-black dark:text-white">
                      {truncateText(customOption.option_type, 20)}
                    </p>
                  </div>
                  ...
                  <div className="col-span-1 hidden items-center sm:flex">
                    <p className="text-sm text-black dark:text-white">
                      <div className="flex items-center space-x-3.5">
                        ...
                        <button
                          className="hover:text-primary"
                          onClick={() => openDeleteModal(customOption.custom_option_id)} // Pass the ID of the item to be deleted
                        >
                          <svg
                            className="fill-current"
                            width="18"
...
export default AllCustomOptions;
